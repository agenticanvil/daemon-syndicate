...
import {
  CAMERA_VIEW_RADIUS,
  angleForCameraDirection,
  cameraPositionForDirection,
  createCameraGizmo,
  lookAtWithStableVerticalUp,
  type AngleId,
} from "./assetEditorCameraGizmo";
...
async function startAssetEditorAsync(app: HTMLDivElement): Promise<void> {
  ...
  const renderer = createRenderer({ preserveDrawingBuffer: true, maxPixelRatio: 2 });
...
function createLoadingMarkup(): string {
  return `<main class="asset-editor-empty"><h1>Asset Preview</h1><p>Loading assets...</p></main>`;
}

function createEmptyMarkup(message: string): string {
  return `<main class="asset-editor-empty"><h1>Asset Preview</h1><p>${escapeHtml(message)}</p></main>`;
}
...
function createAssetEditorMarkup(state: AssetEditorState, records: EditorAssetRecord[]): string {
  ...
  return `
    <main class="asset-editor-shell">
      ...
      <aside class="asset-editor-panel" aria-label="Asset preview controls">
        <div class="asset-editor-title"><h1>Asset Preview</h1></div>
